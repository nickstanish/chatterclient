...
public class ChatterClient extends Client {
  /**
   * GUI based client for vizbits chatterbox instant messenger Use JDK and JRE 7!
   * 
   * @author: Nick Stanish
   * @author: Joey Imburgia
   */
  /*
   * TODO: notify tab of new messages, only show other contacts in tab title, notify of client
   * offline, fix duplicate convos. ie. {nick, joey} is different then {joey,nick} according to
   * Arrays.equal if array1.contains(array2) and length = length, then same
   */
  ...
  /*
   * bugs to fix: threads don't synchro connectionslist slow sending/receieving error with username
   * in use when it isnt net.vizbits.chatterclient.contacts list ui validate login for illegal chars
   */
  ...
  public ChatterClient(String host, int port) {
    ...
    /*
     * keyboard shortcuts first connect the key to an actionmap then create an action then connect
     * actionmap to the action
     */

    // CTRL + L to login advanced
    mainPanel.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
        KeyStroke.getKeyStroke(KeyEvent.VK_L, InputEvent.CTRL_DOWN_MASK), "advanced");
  ...
  private void showOptions() {
    ...
    int answer =
        JOptionPane.showConfirmDialog(window, optionsPanel, "Options",
  ...
  public void interpretMessage(String s, SimpleAttributeSet style) {
    if (!s.equals("")) {
      ...
      if (len >= 2 && s.substring(0, 2).equals("./")) {
        ...
        if (convo == null)
          sendMessage(null, s, style);
        else
          sendMessage(convo, s, style);
      }
    }
  }
  ...
  private void createLoginScreen() {
    ...
    /*
     * use getPassword to get the password instead of getText this returns a char array this focus
     * listener clears the password field when you click it then if you don't type anything, it will
     * bring the password back when you remove focus
     */
    ...
    /*
     * show login screen without advanced net.vizbits.chatterclient.options require shortcut to show
     * them
     */
